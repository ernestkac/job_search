...
interface CoverLetterModalProps {
  ...
  onOpenApplicationPortal?: (currentLetterText: string) => void;
}
      ...
      <div className="bg-white rounded-3xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-[#D4D3C9] flex flex-col my-auto">
        ...
        <div className="p-6 space-y-4 flex-1">
            ...
            <div className="flex items-center space-x-2">
              ...
              >
                {isCopied ? (
                  ...
                  <Copy className="h-3.5 w-3.5 text-[#5A5A40]" />
                )}
          ...
          <div className="p-3 bg-[#E5E5DF]/70 border border-[#D4D3C9] rounded-2xl text-xs text-[#2D2D2A] flex items-center space-x-2">
            ...
            <span>
              <strong className="font-bold text-[#5A5A40]">
                Review Required:
              </strong>{" "}
              Please review and edit this application letter before sending.
            </span>
          </div>
        ...
        <div className="p-4 bg-[#F8F7F4] border-t border-[#D4D3C9] flex flex-wrap items-center justify-between gap-3 sticky bottom-0 z-10 rounded-b-3xl">
          <button
            onClick={onClose}
            className="px-4 py-2.5 bg-[#E5E5DF] hover:bg-[#D4D3C9] text-[#2D2D2A] font-bold text-xs uppercase tracking-wider rounded-xl transition"
          >
            Close Window
          </button>
          ...
          {onOpenApplicationPortal && (
            <button
              ...
              onClick={() => {
                onClose();
                onOpenApplicationPortal(currentLetter);
              }}
              className="px-5 py-2.5 bg-[#5A5A40] hover:bg-[#4A4A35] text-white font-bold text-xs uppercase tracking-wider rounded-xl transition flex items-center space-x-2 shadow-md"
            >
              <Send className="h-4 w-4" />
              <span>Apply Now via Gmail (One-Click)</span>
            </button>
          )}
        </div>
      </div>
