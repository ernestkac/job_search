...
import { FileText, Calendar, Mail, ExternalLink, Edit3, Trash2, CheckCircle2, Clock, Sparkles, Filter } from 'lucide-react';
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[#2D2D2A]/60 text-xs">
            Track saved job opportunities, tailored letters, submission contacts, and follow-up reminders.
          </p>
        </div>
        ...
        <div className="flex items-center space-x-2 bg-white px-3 py-2 rounded-xl border border-[#D4D3C9] text-xs">
          ...
          <span className="font-bold text-[#5A5A40] uppercase tracking-wider text-[10px]">Filter Stage:</span>
          <select
      ...
      {filteredApps.length === 0 ? (
        <div className="bg-white rounded-3xl p-12 text-center border border-[#D4D3C9] space-y-3">
          ...
          <h3 className="font-serif font-bold text-[#2D2D2A] text-sm">No applications tracked in this stage</h3>
          <p className="text-xs text-[#2D2D2A]/60 max-w-sm mx-auto">
            Save job listings from the Vacancies tab or generate application letters to automatically add them here.
          </p>
        </div>
      ) : (
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="space-y-1">
                  <div className="flex items-center space-x-2">
                    <span className="text-sm font-serif font-bold text-[#2D2D2A]">{app.jobTitle}</span>
                  </div>
                  <p className="text-xs font-bold text-[#5A5A40]">{app.employer}</p>
              ...
              <div className="p-4 bg-[#F8F7F4] rounded-2xl border border-[#D4D3C9] space-y-3 text-xs">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#D4D3C9] pb-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-bold text-[#2D2D2A]">Application Letter:</span>
                    {app.generatedLetter ? (
                      ...
                      <span className="text-[#2D2D2A]/50 italic text-[11px]">Not generated yet</span>
                    )}
                  </div>
                  ...
                  {app.generatedLetter && (
                    <button
                      ...
                      className="text-xs font-bold text-[#5A5A40] hover:text-[#2D2D2A] flex items-center space-x-1 uppercase tracking-wider"
                    >
                  ...
                  <div className="space-y-3 pt-1">
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="font-bold text-[#5A5A40] text-[10px] uppercase tracking-wider">Follow-up Date:</span>
                        <input
                          type="date"
                          value={tempReminder}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="space-y-1">
                      <p className="text-[#2D2D2A] leading-relaxed">
                        <span className="font-bold text-[#5A5A40]">Notes: </span>
                        {app.notes || 'No specific notes recorded for this application.'}
                      </p>
                      {app.followUpReminderDate && (
                        <p className="text-amber-900 font-bold flex items-center space-x-1 text-[11px]">
                          <Clock className="h-3 w-3 text-amber-700" />
                          <span>Follow-up scheduled: {app.followUpReminderDate}</span>
                        </p>
                      )}
                    </div>
